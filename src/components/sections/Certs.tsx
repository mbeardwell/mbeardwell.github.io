...
const TL = [
  ...
  { label: "Security Blue Team OSINT",		date: "Jun 2025",    status: "done"     },
...
function Timeline() {
  ...
  return (
    <div ref={r} className="relative mb-14">
      ...
      <div className="absolute top-[6px] left-[12.5%] h-px bg-[var(--green)]" style={{ width: "calc(75% * 0.87)" }} />
      <div className="flex justify-around">
        {TL.map((item) => (
          <div key={item.label} className="flex flex-col items-center gap-2.5 flex-1">
            <div
            ...
            <div className="text-center max-w-[110px]">
              <div className="text-[10px] tracking-[0.06em] font-medium mb-1" style={{ color: dotColor(item.status) }}>
                {item.date}
              </div>
              <div className="text-[11px] text-[var(--mu)] leading-[1.35]">{item.label}</div>
            </div>
          </div>
        ))}
      </div>
...
function StatusDot({ status }: { status: CertStatus }) {
  return (
    <span
      ...
      style={{ background: dotColor(status), boxShadow: `0 0 6px ${dotColor(status)}` }}
    />
  );
}
      ...
      <div className="relative overflow-hidden h-[90px] flex-shrink-0">
        {certFile ? (
          ...
          <div
            ...
            style={{ background: "linear-gradient(135deg, color-mix(in srgb, var(--amber) 12%, var(--surface)), color-mix(in srgb, var(--ac) 8%, var(--surface)))" }}
          >
            <IconCert className="w-10 h-10 opacity-20" style={{ color: "var(--amber)" }} />
          </div>
        )}
      </div>
      <div className="flex flex-col gap-1.5 p-5 flex-1">
        <div className="flex justify-between items-center gap-2 mb-1 min-w-0">
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <span className="text-[9px] tracking-[0.14em] uppercase text-[var(--mu)] truncate whitespace-nowrap">{issuer}</span>
          </div>
          <span className="text-[9px] tracking-[0.1em] uppercase font-semibold flex-shrink-0" style={{ color: dotColor(status) }}>
            {status === "done" ? "Achieved" : "In Progress"}
          </span>
        </div>
        <p className="font-serif font-normal text-[16px] text-content leading-[1.3]">{name}</p>
        <p className="text-[11px] text-[var(--mu)] mt-auto pt-2">{dateIssued}</p>
      </div>
    </div>
  );

  return certFile ? (
    <a ref={r} href={`./docs/certs/${certFile}.pdf`} target="_blank" rel="noopener noreferrer" className="no-underline text-inherit block h-full">
      {inner}
    </a>
  ) : (
    <div ref={r} className="h-full">{inner}</div>
  );
}
  ...
  return (
    <Section id="certs">
      <div className="flex flex-col gap-10">
        <p ref={r} className="text-[10px] font-medium tracking-[0.2em] uppercase text-accent">Certifications</p>
        <Timeline />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {CERTS.map(c => <Cert key={c.name} {...c} />)}
        </div>
      </div>
    </Section>
  );
