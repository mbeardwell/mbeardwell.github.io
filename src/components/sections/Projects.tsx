...
function Project({ n, title, description, tags, link }: ProjectProps) {
	...
	return (
		...
		>
			<div className="flex justify-between items-start gap-3 mb-3">
				...
				<div className="flex gap-1.5 flex-wrap justify-end pt-1">
					{tags.map((tag) => (
						<span
							key={tag}
							className="text-[9px] tracking-[0.1em] uppercase text-accent border border-[var(--bd)] px-2 py-0.5"
						>
							{tag}
						</span>
					))}
			...
			<h3 className="font-serif font-normal text-[18px] text-content leading-[1.35] mb-3">
				{title}
			</h3>
			<p className="text-[13px] text-[var(--mu)] leading-[1.72] mb-4">
				{description}
			</p>
...
const PROJECTS: ProjectProps[] = [
	{
		...
		description:
			"TCP connect port scanner using multiprocessing — validates user input, grabs banners, and writes results to file. Demonstrates socket programming, argparse, regex, and parallel scanning.",
	...
	},
	{
		...
		description:
			"Tracked real-world identity from anonymised YouTube uploads. Shows attacker thinking, threat profiling, and threat intelligence fundamentals: collection, enrichment, analysis, and attribution.",
	...
	},
	{
		...
		description:
			'Curation of UK-only, legal person-focused OSINT resources. Featured by "Learn Cybersecurity" Telegram (27k subscribers); 100+ clicks in one week.',
	...
	},
	{
		...
		description:
			"Runtime patching tool on ARM Linux to redirect processor instructions in memory, mirroring techniques used in evasive malware. Demonstrates low-level security research fundamentals.",
